...
def generate_V_real(P,dP,Q,rho,v,D,L,n,K,eta_0,eta_inf,tau_0,lambda_,a,P_amb,debug_mode):
    ...
    for i in range(D.shape[1]):
        ...
        while variation_Q >= dQ_crit:
            ...
            SR_temp *= rabi
            eta_temp, deta_temp = calculateVisco.calculateVisco (SR_temp, n, K, eta_inf,eta_0,tau_0, lambda_, a, debug_mode, 0)
            _,Ri = calculateReq.calculateReq(eta_temp, L[0], D[:,i])
            Ri *= rabi
            _, dRi = calculateReqError.calculateReqError(0, Ri, 1, D[:,i], L[0],eta_temp, deta_temp)
            Q_temp = (P - P_amb)/Ri
        ...
        Q_real[i] = Q_temp
        Q_theo[i] = (np.pi * n / (3 * n + 1)) * (D[0, i] / 2)**((1 + 3 * n) / n) * ((P - P_amb + rho * 9.81 * L[0] / 1000) / (2 * K * L[0]))**(1/n)
...
# # def calculateSR(Q_guess, D, param):
# #     # Calculate shear rate
# #     pass

# def calculateVisco(SR_temp, n, K, eta_inf, eta_0, tau_0, lambda_, a, debug_mode, param):
#     # Calculate viscosity
#     pass

# def calculateReq(eta_temp, L, D):
#     # Calculate flow resistance
#     pass

# def calculateReqError(param1, Ri, param2, D, L, eta_temp, deta_temp):
#     # Calculate error in flow resistance
#     pass
...
# def validateReynolds(rho, v_real, D, eta_real, debug_mode):
