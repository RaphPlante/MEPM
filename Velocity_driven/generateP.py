...
def generateP(rho, v, D, L, n, K, eta_0, eta_inf, tau_0, lmbda, a, P_amb, debug_mode):
    """
    generateP function's purpose is to regroup all the necessary function calls in order to calculate
    the required pressure for a given material and nozzle exit velocity. It validates the Reynold numbers.
    Finally, it returns the required Pressure along with Viscosities and Shear rates arrays for plots.
    ...
    if typeEcoul == 0:  # Laminar flow
        ...
        R_eq = R_eq * rabi
        Ri = Ri * rabi
        R_eq_error, dRi = calculateReqError(
