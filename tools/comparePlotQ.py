...
def comparePlotQ(v, Q, v_literature=None, Q_literature=None, x_lim=None, y_lim=None, dQ=None, mode='default', my_plot=None):
    if my_plot is None:
    ...
    if Q is not None and v is not None:
        my_plot.loglog(v, Q, '-s', linewidth=1.5, color=red,
                       label='$P_{model}$' if mode == 'latex' else 'P_{model}')
        if np.any(dQ):
            my_plot.errorbar(v, Q, yerr=dQ,
                             fmt='o', color=red, linewidth=1)

        for x, y in zip(v, Q):
            my_plot.text(x * 0.86, y * 1.1,  # Offset text: 10% to the right and 10% below
                         f'{y:.3f}', fontsize=12,
    ...
    my_plot.set_ylabel('Total mass flow rate [g/s]', fontsize=12)
